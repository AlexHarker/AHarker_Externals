...
{	
	...
	#ifdef REVERSE
	float *used_input1 = sp[1]->s_vec;
	float *used_input2 = sp[0]->s_vec;
	#else
	float *used_input1 = sp[0]->s_vec;
	float *used_input2 = sp[1]->s_vec;
	#endif
	...
	if (vec_size_val >= 4 && SSE2_check())
	{
		...
		if ((long) used_input1 % 16 || (long) used_input2 % 16 || (long) sp[2]->s_vec % 16)	
		{
